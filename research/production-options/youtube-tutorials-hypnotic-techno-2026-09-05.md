...
1. Build a "typical FM bass": Oscillator A course ~0.5, Oscillator B course ~3 (donk-style FM bass starting point); adjust ratio to taste.
2. Add EQ, roll off lows at the end of the chain, apply general processing to "keep the sound interesting."
3. Assign multiple LFOs (Live 12's Operator supports several assignable slots): LFO 1 → envelope Attack (small depth, offset brought down); LFO 2 → envelope Sustain (similarly restrained rate/depth); a small Attack increase on Operator A avoids a click at note-on; Attack set toward 0% with amount ~5% avoids clicking with the kick's own transient.
4. LFO on oscillator Level using Random-with-Smooth mode; set floor around 30% and ceiling around 70% so the bass never falls to total silence or gets "too harsh."
5. Additional LFO modulating LFO Rate itself, small range (roughly 20 to 51 sync value mentioned, kept low overall) for extra randomized variation without a "huge amount of movement."
